    ...
    router
        ...
        .put(temp_router_url + '/:id', async (req, res) => {
            ...
            if (req.session.isLogin !== true) {
                ...
                updateSetting.id = Number(updateSetting.id);
                if (!positiveReg.test(updateID) || updateID !== updateSetting.id || updateSetting.id == '' || updateSetting.name == '' || updateSetting.value == '') {
                    res.json({
